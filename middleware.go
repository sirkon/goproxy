...
func (m middleware) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		m.logger.Error().Timestamp().Err(err).Str("prefix", m.prefix).Msg("wrong request")
	...
	if factory == nil {
		...
		(&tmpLogger).Error().Timestamp().Msgf("no proxy handlers registered for %s", path)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		tmpLogger := logger.Logger()
		(&tmpLogger).Error().Timestamp().Err(err)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
	...
	case suffix == "list":
		...
		if err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to get version list")
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		w.WriteHeader(http.StatusOK)
		if _, err := io.WriteString(w, strings.Join(version, "\n")); err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to write list response")
		}
	case strings.HasSuffix(suffix, ".info"):
		version := getVersion(suffix)
		tmpLogger := logger.Str("version", version).Logger()
		ctx := (&tmpLogger).WithContext(req.Context())
		(&tmpLogger).Debug().Timestamp().Msg("version info requested")
		info, err := src.Stat(ctx, version)
		if err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to get revision info from source beneath")
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		je := json.NewEncoder(w)
		if err := je.Encode(info); err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to write version info response")
		}
	case strings.HasSuffix(suffix, ".mod"):
		version := getVersion(suffix)
		tmpLogger := logger.Str("version", version).Logger()
		ctx := (&tmpLogger).WithContext(req.Context())
		(&tmpLogger).Debug().Timestamp().Msg("go.mod requested")
		gomod, err := src.GoMod(ctx, version)
		if err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to get go.mod from a source beneath")
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		if _, err := w.Write(gomod); err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to return go.mod")
			return
		}
	case strings.HasSuffix(suffix, ".zip"):
		...
		tmpLogger := logger.Str("version", version).Logger()
		ctx := (&tmpLogger).WithContext(req.Context())
		(&tmpLogger).Debug().Timestamp().Msg("zip archive requested")
		archiveReader, err := src.Zip(ctx, version)
		if err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to get zip archive")
			w.WriteHeader(http.StatusBadRequest)
			return
		}
		defer func() {
			if err := archiveReader.Close(); err != nil {
				zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msgf("failed to close zip reachive reader")
			}
		}()
		if _, err := io.Copy(w, archiveReader); err != nil {
			zerolog.Ctx(ctx).Error().Timestamp().Err(err).Msg("failed to return zip archive")
		}
	default:
		tmpLogger := logger.Logger()
		(&tmpLogger).Error().Timestamp().Msgf("unsupported suffix %s", suffix)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
}
